...
export const useTaskMeshStore = create<State>((set) => ({
  ...read(),
  addNode: (type) => set((s) => {
  ...
  connectNodes: (c) => set((s) => { const edge: TaskMeshEdge = { ...c, id: `e-${crypto.randomUUID()}`, data: { type: 'relates_to' as EdgeType, label: 'relates_to', description: '' }, markerEnd: { type: MarkerType.ArrowClosed } }; const next = { ...s, edges: addEdge(edge, s.edges) }; persist(next); return next; }),
  ...
  updateEdge: (id, patch) => set((s) => { const next = { ...s, edges: s.edges.map((e) => e.id === id ? { ...e, data: { ...e.data, ...patch } as TaskMeshEdge['data'], label: patch?.label ?? e.label } : e) }; persist(next); return next; }),
